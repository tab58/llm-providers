...
- **Construction:** exported `Config` struct + `NewClient(cfg, opts...)`; functional options only where variation exists (currently `WithNoRateLimit`). Empty `Config` fields get sensible defaults (model, base URL). `NewClient` returns `common.LLM` — the client wrapped with its default limiter — while `WithNoRateLimit()` returns the raw `*Client` (tests needing the concrete type construct with it and type-assert).
